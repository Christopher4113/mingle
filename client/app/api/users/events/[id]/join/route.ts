    ...
    return NextResponse.json(
      await db.$transaction(async (tx) => {
        ...
        if (ev.inviteOnly) {
          ...
          // Notify creator that user requested to join (include event title)
          const notif = await tx.notification.create({
            data: {
              userId: ev.userId,
              type: "EVENT_UPDATE",
              title: "Join Request",
              message: `${actorUsername} requested to join "${ev.title}".`,
              data: {
                eventId: id,
                eventTitle: ev.title,
                actorId: userId,
        ...
        // If not attending already, set ATTENDING and increment attendees
